...
var_dump(class_exists( 'WC_Shortcode_Products' ));
if( !class_exists( 'WPT_Shortcode_Products' ) ){
    class WPT_Shortcode_Products extends WC_Shortcode_Products{
        /**
	 * Shortcode type.
	 *
	 * @since 3.2.0
	...
	public function __construct( $attributes = array(), $type = 'products' ) {
		...
		$this->query_args = $this->parse_query_args();
                //var_dump($this->type,$this->attributes,$this->query_args);
	}
	...
	protected function parse_attributes( $attributes ) {
		...
		$attributes = shortcode_atts(
			array(
				...
				'order'          => '',        // ASC or DESC.
				'ids'            => '',        // Comma separated IDs. //Added
				'skus'           => '',        // Comma separated SKUs.
				'category'       => '',        // Comma separated category slugs or ids. //Added
				'cat_operator'   => 'IN',      // Operator to compare categories. Possible values are 'IN', 'NOT IN', 'AND'. //Added
	...
	protected function set_ids_query_args( &$query_args ) {
		if ( ! empty( $this->attributes['ids'] ) ) {
			$ids = array_map( 'trim', explode( ',', $this->attributes['ids'] ) ); //$this->attributes['ids'];//
	...
	protected function get_query_results() {
		...
		if ( isset( $transient_value['value'], $transient_value['version'] ) && $transient_value['version'] === $transient_version ) {
			$results = $transient_value['value'];
                        ...
                        $this_query_args = $this->query_args;
			$query = new WP_Query( $this->query_args );

			$paginated = ! $query->get( 'no_found_rows' );
                        
                        $ids = empty( $_GET ) && isset($this_query_args['post__in']) && count( $this_query_args['post__in'] ) > 0 ? $this_query_args['post__in'] : $query->posts;
                        //var_dump(wp_parse_id_list($this_query_args['post__in']));
                        //var_dump($this->attributes, $this->get_attributes(), $this->query_args,wp_parse_id_list( $query->posts ),$query->posts);
			...
			if ( $cache ) {
				...
				set_transient( $transient_name, $transient_value, 100 ); //DAY_IN_SECONDS * 
			}
		}
